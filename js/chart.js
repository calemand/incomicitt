...
var xScale = d3.scale.linear().domain([0, 400]).range([0, width]),
    yScale = d3.scale.linear().domain([10, 1500]).range([height, 0]),
    radiusScale = d3.scale.sqrt().domain([0, 5e8]).range([0, 120]),
...
svg.append("text")
    ...
    .attr("x", width)
    .attr("y", height - 6)
    .text(" Inversión en I+D (millones $)");
// Add a y-axis label.
svg.append("text")
    .attr("class", "y label")
    .attr("text-anchor", "end")
    ...
    .text("Inversión en ACT (millones $)");
...
d3.json("./json/nations.json", function(nations) {
  ...
  function init() {
    dragit.init(".gRoot");
    dragit.time = {min:2009, max:2014, step:1, current:2009}
    dragit.data = d3.range(nations.length).map(function() { return Array(); })
    for(var yy = 2009; yy<2014; yy++) {
...
/*
function play_demo() {
  var ex_nations = ["Sector Público", "Sector Académico", "Org. Sin fines de lucro"]
