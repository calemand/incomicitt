...
BubbleTree.Bubbles.Icon = function (node, bubblechart, origin, radius, angle, color) {
    ...
    me.show = function () {
     //   console.log(this);
        ...
        me.circle = me.paper.circle(cx, cy, r)
            .attr({stroke: false, fill: me.color, 'stroke-width': me.strokewidth});

        me.dashedBorder = me.paper.circle(cx, cy, Math.min(r - 3, r * 0.95))
            .attr({stroke: '#ffffff', 'stroke-dasharray': " ",  "stroke-width": me.node.strokewidth, stroke: me.node.stroke});
        ...
        // additional label
        me.label2 = $('<div class="label2 ' + me.node.id + '"><div class="amount"> $' + utils.formatNumber(me.node.amount) + '</div><span>' + me.node.shortLabel + '</span></div>');
    ...
    me.addOverlay = function () {
        ...
        if (Raphael.svg) {
            ...
            if(me.node.children.length>0){
                 me.overlay.node.setAttribute('class', me.node.id+' drilldown');
            }
        }
    ...
    me.draw = function () {
        ...
        if (!showLabel) {
            me.label.hide();
            me.label2.show();
            if ((showIcon && r > 130) || (!showIcon && r > 110)) {
                me.label.show();
                me.label2.hide();
            }else{
                me.label.hide();
      ...
      //   me.label2.show();
        ...
        if (me.hasIcon) {
            if (showIcon) {
                scale = (r - (showLabel ? me.label.height() * 0.5 : 0)) / 70;
